...
def extract_pdf_chunks(
    ...
    # 텍스트 규정집은 페이지별로 나누지 않고 문서 전체에서
    # 조문 경계를 찾는다. 이렇게 해야 조문이 다음 페이지로
    # 이어지는 경우에도 하나의 청크로 유지된다.
    if extraction_mode == "text":
        regulation_chunks = chunk_regulation_pages(
            PageText(
        ...
        for chunk_index, chunk in enumerate(regulation_chunks):
        ...
        print(
            f"  - 문서 전체: {len(regulation_chunks)}개 조문 청크"
        )
        return records
