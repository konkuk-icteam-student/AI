...
from app.ingestion.ocr import ocr_pdf_page


def normalize_direct_text(text: str) -> str:
    """
    텍스트 PDF의 직접 추출 결과를 정규화한다.

    조문 머리는 보통 새 줄에서 시작하므로 줄 경계를 유지한다.
    이전처럼 문서 전체를 한 줄로 합치면 "제5조에 따라"와
    실제 "제5조(절차)"를 안정적으로 구분할 수 없다.
    """
    if not text:
        return ""
